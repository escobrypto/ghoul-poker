...
| P4 | **Environmental polish** | 🟨 PAINTED PERSPECTIVE SHIPPED | The empty-room render is now the stage floor (assets/tables/midnight-room.jpg, 315KB) — stage redefined to 1280x720 matching the render 1:1, all CSS room layers deleted, felt is an invisible anchor traced to the painted LED ring, every element verified against the art. THIS IS THE TABLE-THEME TEMPLATE: each future theme (Cathedral, Crypt, Back Alley…) = one empty-room render + one config entry via P2. Earlier:  DEPTH: seats scale by distance (far small/near big), board cards tilt on the surface w/ contact-shadow pool, your hand fans like held cards, felt gains a front lip + floor shadow. AWAITING FROM ESCO: one high-res EMPTY-ROOM render (mock scene, no cards/panels/avatars/UI, clean dark felt, 2560x1440+) — becomes the perspective stage floor; every future table theme = another empty-room render. Earlier:  Esco's table mock adopted as the visual target. Shipped: community cards to mock proportions (100x144), hole cards 58x82, pot rebuilt as a neon marquee banner with REAL denomination chips (1K/500/100/25/5 cropped from the mock) shown per pot size, side panels slimmed 300/320→266/286 (bigger stage on every screen), chat compacted to the mock's clean density. NEXT PASS (needs Esco's eyes on pass 1 first): seat pod restructure (large avatar art over nameplate, cards below), action bar redesign (hotkey hints, bet fraction buttons, slider, circular timer), top bar restyle. Original scope: | premium casino ambience + backgrounds, GG branding in the table/felt, bigger logo presence, better composition, animation toggle in settings (expose existing GPU gate). |
